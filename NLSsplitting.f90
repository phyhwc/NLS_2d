    ...
    ! i*u_t+Es*|u|^2u+u_{xx}+u_{yy}=0 changed into i*u_t+Es*|u|^2u+alpha*u_{xx}+beta*u_{yy}=0
    ...
    IMPLICIT NONE 
    ! Declare variables
    INTEGER(kind=4), PARAMETER ::  Nx=150
    INTEGER(kind=4), PARAMETER ::  Ny=166
    INTEGER(kind=4), PARAMETER ::  Nt=200000
    INTEGER(kind=4), PARAMETER ::  plotgap=500
    REAL(kind=8), PARAMETER    ::  &
    pi=3.14159265358979323846264338327950288419716939937510d0
    REAL(kind=8), PARAMETER    ::  Lx=200.0d0/pi
    REAL(kind=8), PARAMETER    ::  Ly=200.0d0/pi
    REAL(kind=8), PARAMETER    ::  Es=-2.0d0*sqrt(2.0d0)
    REAL(kind=8), PARAMETER    ::  alpha=4.0d0*sqrt(2.0d0)
    REAL(kind=8), PARAMETER    ::  beta=6.0d0*sqrt(2.0d0)
    REAL(kind=8)               ::  dt=0.001d0
    REAL(kind=8), PARAMETER    ::  Vx=3.0d0
    REAL(kind=8), PARAMETER    ::  Vy=3.0d0
    REAL(kind=8), PARAMETER    ::  c0=4.0d0
    REAL(kind=8), PARAMETER    ::  rho0=1.0d0
    REAL(kind=8), PARAMETER    ::  t=2.0d0
    ...
    unax(1:Nx,j) = sqrt(rho0 - 0.4D1 / ((-Vx * t - c0 * t + x(1:Nx)) * (&
     -Vy * t + y(j)) * Vy / 0.3D1 + (-Vy * t + y(j)) ** 2 * Vx / 0.3D1 + (-Vx&
      * t - c0 * t + x(1:Nx)) ** 2 + 0.36D2 / (-Vy ** 2 + 0.12D2 * Vx)) + 0.2&
     D1 * ((-Vy * t + y(j)) * Vy / 0.3D1 - 0.2D1 * Vx * t - 0.2D1 * c0 * t&
      + 0.2D1 * x(1:Nx)) ** 2 / ((-Vx * t - c0 * t + x(1:Nx)) * (-Vy * t + y(j)) * Vy &
     / 0.3D1 + (-Vy * t + y(j)) ** 2 * Vx / 0.3D1 + (-Vx * t - c0 * t + x(1:Nx))&
      ** 2 + 0.36D2 / (-Vy ** 2 + 0.12D2 * Vx)) ** 2)
    ...
    DO j=1,Ny
            !DO i=1,Nx
                    !WRITE(11,*) abs(unax(i,j))**2
                      ...
                      write(11,'(1x,512e14.4)') (abs(unax(i,j))**2,i=1,Nx)
    ...
    DO n=1,Nt
            ...
            DO j=1,Ny
                    DO i=1,Nx
                            vnax(i,j)=exp(0.5d0*dt*(alpha*kx(i)*kx(i) + beta*ky(j)*ky(j))&
                                            *cmplx(0.0d0,1.0d0))*vnax(i,j)
                    END DO
            END DO
            !$OMP END PARALLEL DO
